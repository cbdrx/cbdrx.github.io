...
var WorldGenColors = ['#C9C165','#0000FF','#00CC00','#FF9900', '#00FFFF','#D2E0DE'];
...
function mapGenRule(grid, x, y)
{
  ...
  else if(grid[x][y] == 2)
  {
    ...
    if(getNumberOfNeighborsNoWrap(grid,x,y,checkForType(3)) > 0 && Math.random() > 0.8)
    {
        return 3;
    }
    //A coastal tile may erode
    else if(getNumberOfNeighborsNoWrap(grid,x,y,checkForType(1)) > 4 && Math.random() > 0.9)
    {
      return 0;
    }
    ...
    else if(getNumberOfNeighborsNoWrap(grid,x,y,checkForType(0)) == 0
    && getNumberOfNeighborsNoWrap(grid,x,y,checkForType(1)) == 0)
    {
      //if a hilltop is near a mountain, it may become a mountain
      if(getNumberOfNeighborsNoWrap(grid,x,y,checkForType(5)) > 0 && Math.random() > 0.999)
      {
        return 5;
      }
      //if a hilltop is totally surrounded by hills, a mountain may form
      else if(getNumberOfNeighborsNoWrap(grid,x,y,checkForType(2)) == 8 && Math.random() > 0.99999)
      {
        return 5;
      }
      else {
  ...
  {
    if(getNumberOfNeighborsNoWrap(grid,x,y,checkForType(4)) > 0 && Math.random() > 0.7)
    {
        return 4;
    }
    else if(getNumberOfNeighborsNoWrap(grid,x,y,checkForType(1)) > 0 && Math.random() > 0.9)
    {
        return 1;
    }
    else {
  ...
  else if(grid[x][y] == 5 && (getNumberOfNeighborsNoWrap(grid,x,y,checkForType(1))
...
function init() {
  ...
  paused = true;
  cellWidth = Math.floor(elementWidth / numCells);
  theGrid = currentGenerator(numCells);
  window.requestAnimationFrame(draw);
  drawSquareAt(0,0,elementWidth,"white");
...
{
  drawSquareAt(x*cellWidth,y*cellWidth,cellWidth,colorsArray[grid[x][y]]);
}
...
function getTimeBetween()
{
  ...
  if( userValue != "")
  {
    if(userValue < 50)
    {
      alert("1 is the minimum ms per frame");
      timePerFrame = 50;
    }
    else {
